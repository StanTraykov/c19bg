...
##### tidy
atab[-1,-1] <- atab[-1,-1] - atab[-nrow(atab),-1] # replace totals w/incidence
...
hplot <- function() {
    plt <- ggplot(data = atab,
        ...
        labs(title = enc("Седмична заболеваемост на COVID-19 (регистрирани нови случаи на 100 хил.)"),
             caption = enc("*дясно подравнена 7-дневна сума спрямо докладваните в понеделник на следващата седмица; данни: data.egov.bg, НСИ"),
             fill = enc("c/100K"),
...
# e.g.:
# plot(hplot())
# ggsave(file = "heat.png", plot = hplot())