...
ggrid <- data.frame( # bg oblast grid from geofacet pkg; modified codes/names
    code = c("VID", "PVN", "DOB", "RSE", "SLS", "SHU",
             ...
             "KNL", "KRZ", "SML", "BLG"),
    name = c(name = c(enc("Видин"), enc("Плевен"), enc("Добрич"), enc("Русе"),
...
otab[-1,-1] <- otab[-1,-1] - otab[-nrow(otab),-1] # replace totals w/incidence
otab <- otab[-1,]
names(otab)[1] <- "date"
otab <- otab %>%
...
oblasts_plot <- function(incid_100K) {
    ...
    plt <- plt +
        ...
        plt <- plt + l + t + facet_geo(~ oblast, grid = ggrid, scales = s)
    return(plt)
}
